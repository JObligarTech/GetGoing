 ...
 */
export function TripMap({ center, zoom = 13, pins = [], path, dark, label, style, interactive = false }: {
  center: { lat: number; lng: number }; zoom?: number; pins?: MapPin[]; path?: LatLng[]; dark?: boolean; label: string; style?: StyleProp<ViewStyle>; interactive?: boolean;
}) {
  const t = useTheme();
  const isDark = dark ?? t.scheme === "dark";
  const html = useMemo(() => buildHtml({ center, zoom, pins, path, dark: isDark, interactive }), [center, zoom, pins, path, isDark, interactive]);
  return (
...
export function buildHtml(o: { center: { lat: number; lng: number }; zoom: number; pins: MapPin[]; path?: LatLng[]; dark: boolean; interactive: boolean }) {
  ...
  // Route line as [lng,lat] pairs; drawn under the markers once the style has loaded.
  const line = json((o.path ?? []).map((p) => [Number(num(p.lng)), Number(num(p.lat))]));
  const lineColor = o.dark ? "#7A5F1F" : "#2F5D3A", casing = o.dark ? "#0F1A13" : "#ffffff";
...
var line=${line};if(line.length>1){map.on('load',function(){map.addSource('r',{type:'geojson',data:{type:'Feature',properties:{},geometry:{type:'LineString',coordinates:line}}});
...
map.addLayer({id:'rl',type:'line',source:'r',paint:{'line-color':'${lineColor}','line-width':5},layout:{'line-cap':'round','line-join':'round'}});
var b=line.reduce(function(a,c){return a.extend(c);},new maplibregl.LngLatBounds(line[0],line[0]));map.fitBounds(b,{padding:56,duration:0,maxZoom:15});});}
</script></body></html>`;
}
