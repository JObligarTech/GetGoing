...
function buildHtml(o: { center: { lat: number; lng: number }; zoom: number; pins: MapPin[]; dark: boolean; interactive: boolean }) {
  ...
  const pins = JSON.stringify(o.pins.map((p) => ({ lat: p.lat, lng: p.lng, label: p.label ?? null, color: p.color, dark: !!p.dark })));
  // No user text is interpolated unescaped: labels go through textContent.
...
<script>
var map=new maplibregl.Map({container:'m',style:{version:8,sources:{osm:{type:'raster',tiles:['https://tile.openstreetmap.org/{z}/{x}/{y}.png'],tileSize:256,maxzoom:19}},layers:[{id:'osm',type:'raster',source:'osm'}]},center:[${o.center.lng},${o.center.lat}],zoom:${o.zoom},interactive:${o.interactive},attributionControl:false});
