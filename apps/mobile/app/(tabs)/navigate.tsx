...
import { formatDateRange, isTree, localDate, MODE_LABEL, navShortcuts, placeById, pluralize, tripDayNumber } from "@voya/core";
      ...
      <ScrollView contentContainerStyle={[s.content, { paddingTop: insets.top + 8 }]}>
        ...
        <Animated.View entering={enter(1)} style={{ gap: 8 }}>
          ...
          <Card>
            ...
            <ListRow onPress={() => router.push({ pathname: "/navigate/tree", params: { day } })} leading={<IconCoin name="git-network-outline" />} title="Plan a tree route" subtitle="Groups split, compare, meet again" chevron />
            <SendLocation travelers={bundle.travelers.filter((x) => x.user_id !== user.id)} senderName={user.profile.display_name.split(" ")[0] ?? "I"} tz={tz} />
          </Card>
        </Animated.View>
          ...
          {bundle.routes.length ? (
            <Card>
              {bundle.routes.map((r, i) => {
                ...
                return (
                  <ListRow key={r.id} last={i === bundle.routes.length - 1} onPress={() => router.push(tree ? { pathname: "/navigate/tree", params: { route: r.id } } : { pathname: "/navigate/day", params: { route: r.id } })} leading={<Tile name={r.name} size={44} radius={12} invert />} title={r.name} subtitle={tree ? `Tree route · ${pluralize(groups, "group")} · ${pluralize(stops.length, "stop")}` : `${pluralize(stops.length, "stop")} · ${MODE_LABEL[r.mode]}${ends}`} trailing={r.day ? <Chip tone="plain">{formatDateRange(r.day, null)}</Chip> : undefined} chevron />
                );
              })}
            </Card>
          ) : (
